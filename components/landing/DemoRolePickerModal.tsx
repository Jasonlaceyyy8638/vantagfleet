...
export function DemoRolePickerModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return createPortal(
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border-2 border-cyber-amber/85 bg-midnight-ink shadow-[0_0_60px_-12px_rgba(255,176,0,0.35)] p-6 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            type="button"
            onClick={() => go('carrier')}
            className="group flex flex-col items-stretch text-left rounded-xl border-2 border-cyber-amber/55 bg-cyber-amber/10 hover:bg-cyber-amber/18 px-5 py-6 transition-colors min-h-[120px]"
          >
            <Truck className="size-8 text-cyber-amber mb-3 shrink-0" aria-hidden />
