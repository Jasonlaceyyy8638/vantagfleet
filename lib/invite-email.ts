...
export function getWelcomeToTeamEmail(
  ...
  const text = [
    ...
    '',
    'After signing in, go to Settings to change your password.',
    '',
    '— VantagFleet',
  ].join('\n');
  ...
  const body = `
          ...
          <p style="margin:0 0 8px;font-size:16px;line-height:1.5;color:${EMAIL_TEXT};"><strong>Your temporary password:</strong></p>
          <p style="margin:0 0 16px;font-size:18px;font-weight:600;color:${EMAIL_AMBER};letter-spacing:0.05em;">${escapeHtml(tempPassword)}</p>
          ...
          <p style="margin:0 0 24px;font-size:14px;color:${EMAIL_MUTED};">After signing in, go to Settings to change your password.</p>
          <p style="margin:0;font-size:14px;color:${EMAIL_MUTED};">— VantagFleet</p>`;
  const html = emailWrapper(body);
