...
export default async function TeamPage() {
  ...
  if (role !== 'Owner' && role !== 'Safety_Manager') redirect('/settings');

  const admin = createAdminClient();
  const { data: profiles } = await admin.from('profiles').select('id, user_id, full_name, role').eq('org_id', orgId).order('role');
  const members: TeamMember[] = [];
  for (const p of profiles ?? []) {
    const row = p as { id: string; user_id: string; full_name: string | null; role: string };
