...
const SYSTEM_PROMPT =
  'You are a DOT Compliance officer. Look at this Medical Card or CDL. Extract the Driver Full Name, the Expiration Date, and the Document Type. Return ONLY valid JSON: { "name": "string", "expirationDate": "YYYY-MM-DD", "type": "MED_CARD" | "CDL" }';

// Use gpt-4o-mini (fast, cost-effective). Change to gpt-5-nano when available.
...
export async function scanDq(formData: FormData): Promise<ScanDqResult> {
  ...
  try {
    ...
    const expirationDate =
      typeof json.expirationDate === 'string' ? json.expirationDate : null;
    const type =
      json.type === 'MED_CARD' || json.type === 'CDL' ? json.type : 'MED_CARD';
    ...
    if (
      ...
      return {
        ok: false,
        error: 'No valid expiration date found on the document.',
      };
    }
...
function extractJson(
  text: string
): { name?: string; expirationDate?: string; type?: string } | null {
